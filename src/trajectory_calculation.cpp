...
std::tuple<MultiWaypointTrajectory, Scalar> imp_calculate_trajectory_cost_and_optimal_velocities(std::vector<int> &scheduled_locations_idx,
  ...
  imp_travel_cost_map& imp_travel_costs = env_params.imp_precalculated_costs;
  std::vector<std::tuple<Vector<3>, Scalar, Scalar>> imp_velocity_sample_tuples = env_params.imp_velocity_samples_tuples;
  ...
  if (sample_start_velocity) {
    ...
    // SHould we put [yaw, ang] insteaad of zeros? TODO(mark)
  ...
  for (size_t loc_to = 1; loc_to < number_of_locations; loc_to++) {
    ...
    for (int sample_idx_from = 0; sample_idx_from < from_shortest_samples_dists.size(); sample_idx_from++) {
      ...
      for (int sample_idx_to = 0; sample_idx_to < to_shortest_samples_dists.size(); sample_idx_to++) {
        ...
        auto pos1 = scheduled_locations_idx[loc_to-1];
        auto pos2 = scheduled_locations_idx[loc_to];
        Scalar time_between = travel_costs[pos1][pos2][from_norm][to_norm][from_angle][to_angle];
        Scalar another_time_between = imp_travel_costs[pos1][pos2][sample_idx_from+1][sample_idx_to+1];
//        if (time_between != another_time_between) {
//          std::cout << "true" << std::endl;
//          std::cout << pos1 << " -> " << pos2 << std::endl;
//          std::cout << "time_between -> " << time_between << std::endl;
//          std::cout << "another_time_between -> " << another_time_between << ", sample_idx_from -> " << sample_idx_from+1 << ", sample_idx_to -> " << sample_idx_from+1 << std::endl;
//        }
