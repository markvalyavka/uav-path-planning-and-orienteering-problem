...
std::tuple<MultiWaypointTrajectory, Scalar> calculate_trajectory_cost_and_optimal_velocities(std::vector<int> &scheduled_locations_idx,
  ...
  const std::vector<std::tuple<Vector<3>, Scalar, Scalar>>& velocity_sample_tuples = env_params.velocity_samples_tuples;
...
//        Scalar time_between = travel_costs[scheduled_locations_idx[loc_to-1]][scheduled_locations_idx[loc_to]][from_norm][to_norm][from_angle][to_angle];
        Scalar time_between = travel_costs.at(scheduled_locations_idx.at(loc_to-1)).at(scheduled_locations_idx.at(loc_to)).at(from_norm).at(to_norm).at(from_angle).at(to_angle);
